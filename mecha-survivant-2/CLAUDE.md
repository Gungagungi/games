...
**Les 29 planches de sprites sont là**, dont cinq boss aujourd'hui retirés du
jeu (gravedigger, bone_colossus, plague, mega_boss, titan — ce dernier
réutilisé tel quel par `titan_decoy.gd`, voir plus haut). Les quatre nouveaux
boss (Chevalier Géant, Monstre des Égouts, Zombie Titan, Boss Galaxie,
`idle`/`attack`, 96 ou 128 px) et le squelette de feu (`walk`/`death`, 32 px)
ont chacun leur planche, générée via Retro Diffusion pour 1,71 $ au total.
**Les 21 bruitages et les 4 musiques aussi** (voir « Audio »).
